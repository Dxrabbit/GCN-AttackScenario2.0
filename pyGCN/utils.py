...
#         for k in range(i+1,Alert_df.shape[0]-i-1):
#             for j in range(Alert_df.shape[1]): 
#                 if (j == 0)|(j == 5):
#                     if ((Alert_df.iloc[i,j] == Alert_df.iloc[i+k+1,j])&(Alert_df.iloc[i,j+1] == Alert_df.iloc[i+k+1,j+1]))|((Alert_df.iloc[i,j] == Alert_df.iloc[i+k+1,j+1])&(Alert_df.iloc[i,j+1] == Alert_df.iloc[i+k+1,j])):
#                         print(i,i+k+1,Alert_df.iloc[i,j])
#                         if j == 0:
#                             alert_threshold = alert_threshold + weight[j]
#                         else:
#                             alert_threshold = alert_threshold + weight[j-1]
#                 elif Alert_df.iloc[i,j] == Alert_df.iloc[i+k+1,j]:
#                     if (j < 5)&(j != 1):
#                         alert_threshold = alert_threshold + weight[j-1]
#                     elif j > 6:
#                         alert_threshold = alert_threshold + weight[j-2]
...
def load_data(path="./data/alert/", dataset="alert"):
    ...
    #读取警报文件
    idx_features_labels = np.loadtxt("{}{}.content".format(path, dataset),dtype=np.dtype(str),delimiter=',',skiprows=1)
    #将警报类型转换成独热编码
    ...
    #读取edges.txt文件
    edges_unordered = np.loadtxt("{}{}.similarity".format(path,dataset), dtype=np.int32,delimiter=',')
...
# dataset="alert"
# idx_features_labels = np.loadtxt("{}{}.content".format(path, dataset),dtype=np.dtype(str),delimiter=',',skiprows=1)
# alert_edge(idx_features_labels[6535:6954,:-1])
